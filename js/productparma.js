document.addEventListener('DOMContentLoaded', () => {
    ...
    const initProgressBar = () => {
        const slider = document.querySelector('.parma-logo-strip');
        const thumb = document.querySelector('.parma-smallprogress-bar');
        const track = document.querySelector('.parma-smallprogress-container');
    ...
    const initHeroSlider = () => {
        const videoEl = document.getElementById('parma-hero-video');
        const imageEl = document.getElementById('parma-hero-image');
        const dotsContainer = document.getElementById('parma-hero-dots');
        const container = document.querySelector('.parma-hero-container');
        const nextBtn = document.querySelector('.parma-next');
        const prevBtn = document.querySelector('.parma-prev');

        const playBtn = document.getElementById('parma-video-play-btn');
        const progressContainer = document.getElementById('parma-video-progress-container');
        const progressBar = document.getElementById('parma-video-progress-bar');
        ...
        if (container) {
            ...
            container.addEventListener('click', (e) => {
                ...
                if (item && item.type === 'video' && window.innerWidth < 1024) {
                    if (e.target.closest('#parma-video-play-btn')) return;
        ...
        const updateDots = (index) => {
            ...
            const dots = dotsContainer.querySelectorAll('.parma-dot');
        ...
        if (dotsContainer) {
            ...
            heroGallery.forEach((_, i) => {
                ...
                dot.className = `parma-dot ${i === 0 ? 'active' : ''}`;
                dot.onclick = (e) => { 
                    e.stopPropagation();
                    currentHeroIndex = i; 
        ...
        container.addEventListener('touchend', (e) => {
            if (e.target.closest('#parma-video-play-btn') || e.target.closest('#parma-video-progress-container')) return;
    ...
    const initGalleryScroll = () => {
        const grid = document.querySelector('.parma-gallery-grid');
        const progressBar = document.querySelector('.parma-progress-bar');
        const container = document.querySelector('.parma-progress-container');
    ...
    const initProductLightbox = () => {
        ...
        const heroContainer = document.querySelector('.parma-hero-container');
        if (heroContainer) {
            heroContainer.style.cursor = "pointer";
            heroContainer.addEventListener('click', (e) => {
                if (e.target.closest('#parma-video-play-btn') || e.target.closest('#parma-video-progress-container')) {
                    return;
                }
        ...
        const gridImages = Array.from(document.querySelectorAll('.parma-gallery-grid .parma-photo img'));
...
const navLinks = document.querySelectorAll('.parma-nav-link');
...
const stickyWrapper = document.querySelector('.parma-sticky-wrapper');
const navContainer = document.querySelector('.parma-sticky-nav-container');
const mainBodyWrapper = document.querySelector('.parma-main-body-wrapper');
