document.addEventListener('DOMContentLoaded', () => {
    ...
    const initProgressBar = () => {
        const slider = document.querySelector('.ladolce-logo-strip');
        const thumb = document.querySelector('.ladolce-smallprogress-bar');
        const track = document.querySelector('.ladolce-smallprogress-container');
    ...
    const initHeroSlider = () => {
        const videoEl = document.getElementById('ladolce-hero-video');
        const imageEl = document.getElementById('ladolce-hero-image');
        const dotsContainer = document.getElementById('ladolce-hero-dots');
        const container = document.querySelector('.ladolce-hero-container');
        const nextBtn = document.querySelector('.ladolce-next');
        const prevBtn = document.querySelector('.ladolce-prev');

        const playBtn = document.getElementById('ladolce-video-play-btn');
        const progressContainer = document.getElementById('ladolce-video-progress-container');
        const progressBar = document.getElementById('ladolce-video-progress-bar');
        ...
        if (container) {
            ...
            container.addEventListener('click', (e) => {
                ...
                if (item && item.type === 'video' && window.innerWidth < 1024) {
                    if (e.target.closest('#ladolce-video-play-btn')) return;
        ...
        const updateDots = (index) => {
            ...
            const dots = dotsContainer.querySelectorAll('.ladolce-dot');
        ...
        if (dotsContainer) {
            ...
            heroGallery.forEach((_, i) => {
                ...
                dot.className = `ladolce-dot ${i === 0 ? 'active' : ''}`;
                dot.onclick = (e) => { 
                    e.stopPropagation();
                    currentHeroIndex = i; 
        ...
        container.addEventListener('touchend', (e) => {
            if (e.target.closest('#ladolce-video-play-btn') || e.target.closest('#ladolce-video-progress-container')) return;
    ...
    const initGalleryScroll = () => {
        const grid = document.querySelector('.ladolce-gallery-grid');
        const progressBar = document.querySelector('.ladolce-progress-bar');
        const container = document.querySelector('.ladolce-progress-container');
    ...
    const initProductLightbox = () => {
        ...
        const heroContainer = document.querySelector('.ladolce-hero-container');
        if (heroContainer) {
            heroContainer.style.cursor = "pointer";
            heroContainer.addEventListener('click', (e) => {
                if (e.target.closest('#ladolce-video-play-btn') || e.target.closest('#ladolce-video-progress-container')) {
                    return;
                }
        ...
        const gridImages = Array.from(document.querySelectorAll('.ladolce-gallery-grid .ladolce-photo img'));
...
const navLinks = document.querySelectorAll('.ladolce-nav-link');
...
navLinks.forEach(link => {
    link.addEventListener('click', function(e) {
        ...
        if (targetElement) {
            ...
            if (targetId === '#specs' || targetId === '#features' ) {
                headerOffset = 90; 
            }
...
const stickyWrapper = document.querySelector('.ladolce-sticky-wrapper');
const navContainer = document.querySelector('.ladolce-sticky-nav-container');
const mainBodyWrapper = document.querySelector('.ladolce-main-body-wrapper');
