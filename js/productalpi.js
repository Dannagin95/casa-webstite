document.addEventListener('DOMContentLoaded', () => {
    ...
    const initProgressBar = () => {
        const slider = document.querySelector('.alpi-logo-strip');
        const thumb = document.querySelector('.alpi-smallprogress-bar');
        const track = document.querySelector('.alpi-smallprogress-container');
    ...
    const initHeroSlider = () => {
        const videoEl = document.getElementById('alpi-hero-video');
        const imageEl = document.getElementById('alpi-hero-image');
        const dotsContainer = document.getElementById('alpi-hero-dots');
        const container = document.querySelector('.alpi-hero-container');
        const nextBtn = document.querySelector('.alpi-next');
        const prevBtn = document.querySelector('.alpi-prev');

        const playBtn = document.getElementById('alpi-video-play-btn');
        const progressContainer = document.getElementById('alpi-video-progress-container');
        const progressBar = document.getElementById('alpi-video-progress-bar');
        ...
        if (container) {
            ...
            container.addEventListener('click', (e) => {
                ...
                if (item && item.type === 'video' && window.innerWidth < 1024) {
                    if (e.target.closest('#alpi-video-play-btn')) return;
        ...
        const updateDots = (index) => {
            ...
            const dots = dotsContainer.querySelectorAll('.alpi-dot');
        ...
        if (dotsContainer) {
            ...
            heroGallery.forEach((_, i) => {
                ...
                dot.className = `alpi-dot ${i === 0 ? 'active' : ''}`;
                dot.onclick = (e) => { 
                    e.stopPropagation();
                    currentHeroIndex = i; 
        ...
        container.addEventListener('touchend', (e) => {
            if (e.target.closest('#alpi-video-play-btn') || e.target.closest('#alpi-video-progress-container')) return;
    ...
    const initGalleryScroll = () => {
        const grid = document.querySelector('.alpi-gallery-grid');
        const progressBar = document.querySelector('.alpi-progress-bar');
        const container = document.querySelector('.alpi-progress-container');
    ...
    const initProductLightbox = () => {
        ...
        const heroContainer = document.querySelector('.alpi-hero-container');
        if (heroContainer) {
            heroContainer.style.cursor = "pointer";
            heroContainer.addEventListener('click', (e) => {
                if (e.target.closest('#alpi-video-play-btn') || e.target.closest('#alpi-video-progress-container')) {
                    return;
                }
        ...
        const gridImages = Array.from(document.querySelectorAll('.alpi-gallery-grid .alpi-photo img'));
...
const navLinks = document.querySelectorAll('.alpi-nav-link');
...
const stickyWrapper = document.querySelector('.alpi-sticky-wrapper');
const navContainer = document.querySelector('.alpi-sticky-nav-container');
const mainBodyWrapper = document.querySelector('.alpi-main-body-wrapper');
